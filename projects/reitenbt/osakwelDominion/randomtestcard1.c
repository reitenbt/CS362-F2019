...
// Random tests for baronCardEffect()
int main()
{
	int i;
	int test_counter;
	...
	fprintf(stderr, "*** BEGIN BARON TESTING ***\n");
	...
	for (test_counter = 0; test_counter < NUMBER_OF_TESTS; test_counter++)
	{
		...
		// Play Baron
		while (baronCardEffect(G, choice, current_player, baron_position) != 0)
		{
		...
		// Soft asserts
		if (choice == 1) // choice is to discard an estate
		{
	...
	fprintf(stderr, "*** END BARON TESTING ***\n");
